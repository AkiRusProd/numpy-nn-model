...
class Model:
    ...
    def add(self, layer):
        if self.layers:
            layer.input_shape = self.layers[
                -1
            ].output_shape  # or layer.input_shape == None
    ...
    def prepare_targets(self, batch_targets):
        ...
        for target in batch_targets:
            ...
            try:
                ...
                last_layer_units_num = self.layers[-1].output_shape[
                    -1
                ]  # NOTE: Units num that correctly works with Last Dense Layer

                if (
                    last_layer_activation == activations["sigmoid"]
                    or last_layer_activation == activations["softmax"]
                ):
    ...
    def fit(self, input_data, data_targets, batch_size, epochs):
        ...
        for i in range(epochs):
            tqdm_range = tqdm(
                enumerate(zip(batches, batches_targets)), total=len(batches)
            )
    ...
    def predict(self, input_data, data_targets):
        ...
        for i, (input, target) in tqdm(
            enumerate(zip(input_data, data_targets)),
            desc="testing",
            total=len(input_data),
        ):
            predictions = self.forward_prop(
                input.reshape(1, *input.shape), training=False
            )
