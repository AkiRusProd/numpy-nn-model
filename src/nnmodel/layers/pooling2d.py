...
class Pooling2D:
    def __init__(
        ...
        self.pool_size = ValuesChecker.check_size2_variable(
            pool_size, variable_name="pool_size", min_acceptable_value=1
        )
        ...
        self.stride = ValuesChecker.check_size2_variable(
            stride, variable_name="pool_stride", min_acceptable_value=1
        )
    ...
    def build(self):
        ...
        if self.padding == "valid":
            ...
            padding_up_down = (self.stride[0] - 1) * (
                self.input_height - 1
            ) + self.dilation[0] * (self.pool_height - 1)
            padding_left_right = (self.stride[1] - 1) * (
                self.input_width - 1
            ) + self.dilation[1] * (self.pool_width - 1)
        ...
        self.output_height = (
            ...
            + self.padding[0]
            + self.padding[1]
            - self.dilation[0] * (self.pool_size[0] - 1)
            - 1
        ) // self.stride[0] + 1
        self.output_width = (
            self.input_shape[2]
            + self.padding[2]
            + self.padding[3]
            - self.dilation[1] * (self.pool_size[1] - 1)
            - 1
        ) // self.stride[1] + 1
        ...
        self.pool_part_area = self.set_dilation_stride(
            self.pool_part_area, self.dilation
        )
    ...
    @staticmethod
    @njit
    def pooling_error_backward_prop(pooling_layer_ind, error, input_shape):

        error = error.reshape(
            (error.shape[0], error.shape[1], error.shape[2] * error.shape[3])
        )

        output_layer_error = np.zeros(
            (input_shape[0], input_shape[1], input_shape[2], input_shape[3])
        )
...
class MaxPooling2D(Pooling2D):
    ...
    def forward_prop(self, X, training):
        ...
        self.pooling_layer, self.pooling_layer_ind = self.pooling(
            self.input_data,
            self.pool_part_area,
            self.input_data.shape,
            self.dilated_pool_height,
            self.dilated_pool_width,
            self.output_height,
            self.output_width,
            self.stride,
            "MaxPooling",
    ...
    def backward_prop(self, error):
        ...
        output_error = self.pooling_error_backward_prop(
            self.pooling_layer_ind, error, self.input_data.shape
        )
