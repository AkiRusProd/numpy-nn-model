...
class BatchNormalization:
    ...
    def build(self):
        ...
        self.vg, self.mg = np.zeros_like(self.gamma), np.zeros_like(
            self.gamma
        )  # optimizers params
        self.vg_hat, self.mg_hat = np.zeros_like(self.gamma), np.zeros_like(
            self.gamma
        )  # optimizers params

        self.vb, self.mb = np.zeros_like(self.gamma), np.zeros_like(
            self.gamma
        )  # optimizers params
        self.vb_hat, self.mb_hat = np.zeros_like(self.gamma), np.zeros_like(
            self.gamma
        )  # optimizers params
    ...
    def forward_prop(self, X, training):
        ...
        if self.moving_var is None:
            self.moving_var = np.mean(X, axis=0)
        ...
        if training == True:
            ...
            self.moving_mean = (
                self.momentum * self.moving_mean + (1.0 - self.momentum) * self.mean
            )
            self.moving_var = (
                self.momentum * self.moving_var + (1.0 - self.momentum) * self.var
            )
        else:
    ...
    def backward_prop(self, error):
        ...
        output_error = (
            ...
            * (
                ...
                - np.sum(error, axis=0)
                - self.X_centered
                * np.power(self.stddev_inv, 2)
                * np.sum(error * self.X_centered, axis=0)
            )
        )
