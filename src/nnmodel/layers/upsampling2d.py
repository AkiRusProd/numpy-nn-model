...
class UpSampling2D:
    ...
    def __init__(self, size=(2, 2), input_shape=None) -> None:
        self.size = ValuesChecker.check_size2_variable(
            size, variable_name="size", min_acceptable_value=1
        )
    ...
    def build(self):
        self.output_shape = (
            self.input_shape[0],
            self.input_shape[1] * self.size[0],
            self.input_shape[2] * self.size[1],
        )
