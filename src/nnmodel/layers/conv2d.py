...
class Conv2D:
    ...
    def __init__(
        ...
        self.kernels_num = ValuesChecker.check_integer_variable(
            kernels_num, "kernels_num"
        )
        self.kernel_shape = ValuesChecker.check_size2_variable(
            kernel_shape, variable_name="kernel_shape", min_acceptable_value=1
        )
        self.input_shape = ValuesChecker.check_input_dim(input_shape, input_dim=3)
        self.padding = ValuesChecker.check_size2_variable(
            padding, variable_name="padding", min_acceptable_value=0
        )
        self.stride = ValuesChecker.check_size2_variable(
            stride, variable_name="stride", min_acceptable_value=1
        )
        self.dilation = ValuesChecker.check_size2_variable(
            dilation, variable_name="dilation", min_acceptable_value=1
        )
    ...
    def build(self):
        ...
        self.w = np.random.normal(
            0,
            pow(self.kernel_height * self.kernel_width, -0.5),
            (
                self.kernels_num,
                self.channels_num,
                self.kernel_height,
        ...
        self.v, self.m = np.zeros_like(self.w), np.zeros_like(
            self.w
        )  # optimizers params
        self.v_hat, self.m_hat = np.zeros_like(self.w), np.zeros_like(
            self.w
        )  # optimizers params

        self.vb, self.mb = np.zeros_like(self.b), np.zeros_like(
            self.b
        )  # optimizers params
        self.vb_hat, self.mb_hat = np.zeros_like(self.b), np.zeros_like(
            self.b
        )  # optimizers params
    ...
    def forward_prop(self, X, training):
        ...
        self.conv_layer = self._forward_prop(
            self.input_data,
            self.w,
            self.b,
            self.batch_size,
                            ...
                            conv_layer[b, k, h, w] += (
                                np.sum(
                                    input_data[
                                        b,
                                        c,
                                        h * stride[0] : h * stride[0] + kernel_height,
                                        w * stride[1] : w * stride[1] + kernel_width,
                                    ]
                                    * weights[k, c]
                                )
                                + bias[k]
    ...
    def backward_prop(self, error):
        ...
        self.grad_w = self.compute_weights_gradients(
            error,
            self.input_data,
            self.w,
            self.batch_size,
        ...
        conv_backprop_error = self._backward_prop(
            error,
            self.w,
            self.batch_size,
            self.channels_num,
        ...
        conv_backprop_error = self.set_padding(
            conv_backprop_error,
            (
                0,
                self.input_height - self.stride_compared_input_height,
                0,
                self.input_width - self.stride_compared_input_width,
            ),
        )
    ...
    def _backward_prop(
        ...
        conv_backprop_error = np.zeros(
            (batch_size, channels_num, input_height, input_width)
        )
        ...
        error_pattern[
            ...
            :,
            kernel_height
            - 1 : stride[0] * conv_height
            - (stride[0] - 1)
            + kernel_height
            - 1,  # kernel_height - 1 : conv_height + kernel_height - 1,
            kernel_width
            - 1 : stride[0] * conv_width
            - (stride[0] - 1)
            + kernel_width
            - 1,  # kernel_width - 1 :  conv_width  + kernel_width - 1,
                    ...
                    for h in range(input_height):
                        for w in range(input_width):
                            ...
                            conv_backprop_error[b, c, h, w] += np.sum(
                                error_pattern[
                                    b, k, h : h + kernel_height, w : w + kernel_width
                                ]
                                * w_rot_180[k, c]
                            )
                        ...
                        for w in range(kernel_width):
                            ...
                            gradient[k, c, h, w] += np.sum(
                                ...
                                * input_data[
                                    b,
                                    c,
                                    h : h + stride[0] * conv_height - (stride[0] - 1),
                                    w : w + stride[1] * conv_width - (stride[1] - 1),
                                ]
                            )
