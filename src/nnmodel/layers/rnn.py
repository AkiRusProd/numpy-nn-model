...
class RNN:
    ...
    def build(self):
        ...
        self.w = np.random.normal(
            0, pow(self.input_size, -0.5), (self.input_size, self.units_num)
        )
        self.wh = np.random.normal(
            0, pow(self.units_num, -0.5), (self.units_num, self.units_num)
        )
        ...
        self.vh, self.mh = np.zeros_like(self.wh), np.zeros_like(
            self.wh
        )  # optimizers params
        self.vh_hat, self.mh_hat = np.zeros_like(self.wh), np.zeros_like(
            self.wh
        )  # optimizers params
        ...
        self.output_shape = (
            ...
            else (self.timesteps, self.units_num)
        )
    ...
    def forward_prop(self, X, training):
        ...
        for t in range(self.timesteps):
            self.states[:, t, :] = self.activation.function(
                np.dot(self.input_data[:, t, :], self.w)
                + np.dot(self.states[:, t - 1, :], self.wh)
                + self.b
            )
    ...
    def backward_prop(self, error):
        ...
        for t in reversed(range(self.timesteps)):
            hidden_delta = (
                next_hidden_delta + error[:, t, :]
            ) * self.activation.derivative(self.states[:, t, :])
    ...
    def update_weights(self, layer_num):
        ...
        self.wh, self.vh, self.mh, self.vh_hat, self.mh_hat = self.optimizer.update(
            self.grad_wh, self.wh, self.vh, self.mh, self.vh_hat, self.mh_hat, layer_num
        )
