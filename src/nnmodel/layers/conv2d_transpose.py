...
class Conv2DTranspose:
    ...
    def __init__(
        ...
        self.output_padding = ValuesChecker.check_size2_variable(
            output_padding, variable_name="output_padding", min_acceptable_value=0
        )
    ...
    def backward_prop(self, error):
        ...
        self.grad_w = self.compute_gradients(
            error,
            self.input_data,
            self.w,
            self.batch_size,
    ...
    def prepare_inputs(self, input_data):
        ...
        temp_out[
            :, :, : temp_strided.shape[2], : temp_strided.shape[3]
        ...
        input_data = self.remove_padding(
    ...
    def prepare_error(self, error):
        ...
        error = padded_error[
            ...
            :,
            self.dilated_kernel_height
            - 1 : padded_error.shape[2]
            - (self.dilated_kernel_height - 1)
            - self.output_padding[0],  # remove kernel padding that we added
            self.dilated_kernel_width
            - 1 : padded_error.shape[3]
            - (self.dilated_kernel_width - 1)
            - self.output_padding[1],
        ].copy()
                    ...
                    for h in range(kernel_height):
                        for w in range(kernel_width):
                            ...
                            gradient[k, c, h, w] += np.sum(
                                error[b, k]
                                * input_data[
                                    b, c, h : h + conv_height, w : w + conv_width
                                ]
                            )
