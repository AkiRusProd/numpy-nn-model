...
class GAN():
    ...
    def prepare_loss_functions(self, loss_function_name):
        if loss_function_name == 'minimax crossentropy':
            ...
            self.discriminator_fake_loss_func = self.discriminator.loss_functions[loss_function_name]
            self.generator_loss_func = self.discriminator.loss_functions[loss_function_name]
    ...
    def generator_backward_prop(self, batch_layers_outputs):
        ...
        for k in range(len(batch_layers_outputs)):
            ...
            discriminator_outputs = self.discriminator.forward_prop(np.array(batch_layers_outputs[k][-1], ndmin = 2))
    ...
    def discriminator_backward_prop(self, real_discr_outs, fake_discr_outs):
        ...
        for k in range(len(real_discr_outs)):
            ...
            #real data backprop
            discriminator_losses = self.discriminator.backward_prop(real_discr_outs[k], self.discriminator_real_loss_func(real_discr_outs[k][-1], self.real_targets))
            batch_layers_losses.append(discriminator_losses)
            batch_layers_outputs.append(real_discr_outs[k])
            
            #fake data backprop
            discriminator_losses = self.discriminator.backward_prop(fake_discr_outs[k], self.discriminator_fake_loss_func(fake_discr_outs[k][-1], self.fake_targets))
            batch_layers_losses.append(discriminator_losses)
            batch_layers_outputs.append(fake_discr_outs[k])

            
        
        self.discriminator.weights_updating(batch_layers_outputs, batch_layers_losses, self.discriminator_optimizer)



    def train(self, real_inputs, epochs, optimizer_name, loss_function_name = 'binary crossentropy', batch_size = 10, **optimizer_params):
        real_inputs = np.asfarray(real_inputs)
        
        batch_num = len(real_inputs) // batch_size
        batches = np.array_split(real_inputs, batch_num)
        noises = np.array_split(np.random.normal(0, 1, (len(real_inputs), self.generator.topology[0]['inputs num'])), batch_num)
        ...
        for i in range(epochs):
            ...
            if self.epoch_prediction == True:
                ...
                for k in range(self.outputs_num_per_one_epoch):
                    one_epoch_outputs.append(self.predict(self.noises_for_gen_per_epoch[k]))  
    ...
    def predict_per_epoch(self, mode = False, outputs_per_epoch = 0):
        ...
        self.noises_for_gen_per_epoch = np.random.normal(0, 1, (self.outputs_num_per_one_epoch, self.generator.topology[0]['inputs num']))
