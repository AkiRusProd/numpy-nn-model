...
class Tensor:
    def __init__(self, val, args=None, op=None, requires_grad=True, dtype = None, device = "cpu"):
        ...
        if isinstance(val, Tensor): # If val is a tensor, copy its attributes
            self.data = self.xp.array(val.data, dtype=val.data.dtype if dtype is None else dtype) 
            self.grad = val.grad
    ...
    def to (self, device):
        ...
        data = xp.array(self.data) if isinstance(self.data, np.ndarray) else xp.array(self.data.get(), dtype=self.data.dtype)
   
        return Tensor(data, requires_grad = self.requires_grad, dtype = self.data.dtype, device = device)

    def detach(self):
        return Tensor(val = self.data, args = self.args, op = self.op, requires_grad = False, dtype = self.data.dtype, device = self.device)
    ...
    def add(self, t):
        ...
        return Tensor(self.data + t.data, args, "add", requires_grad=requires_grad, device=self.device)
    ...
    def sub(self, t):
        ...
        return Tensor(self.data - t.data, args, "sub", requires_grad=requires_grad, device=self.device)
    ...
    def mul(self, t):
        ...
        return Tensor(self.data * t.data, args, "mul", requires_grad=requires_grad, device=self.device)
    ...
    def div(self, t):
        ...
        return Tensor(self.data / t.data, args, "div", requires_grad=requires_grad, device=self.device)
    ...
    def matmul(self, n):
        ...
        return Tensor(self.xp.matmul(self.data, n.data), args, "matmul", requires_grad=requires_grad, device=self.device)

    def sum(self, *args, **kwargs):
        axis = kwargs.get("axis", None) if len(args) == 0 else args[0]
        return Tensor(self.data.sum(*args, **kwargs), [self, axis], "sum", requires_grad=self.requires_grad, device=self.device)

    def mean(self, *args, **kwargs):
        axis = kwargs.get("axis", None) if len(args) == 0 else args[0]
        return Tensor(self.data.mean(*args, **kwargs), [self, axis], "mean", requires_grad=self.requires_grad, device=self.device)

    def var(self, *args, **kwargs):
        axis = kwargs.get("axis", None) if len(args) == 0 else args[0]
        return Tensor(self.data.var(*args, **kwargs), [self, axis], "var", requires_grad=self.requires_grad, device=self.device) #ddof = 0;
    ...
    def power(self, n):
        ...
        return Tensor(self.data ** n.data, args, "power", requires_grad=requires_grad, device=self.device)    

    def sqrt(self):
        return Tensor(self.xp.sqrt(self.data), [self], "sqrt", requires_grad=self.requires_grad, device=self.device)

    def log(self):
        return Tensor(self.xp.log(self.data), [self], "log", requires_grad=self.requires_grad, device=self.device)

    def exp(self):
        return Tensor(self.xp.exp(self.data), [self], "exp", requires_grad=self.requires_grad, device=self.device)

    def tanh(self):
        return Tensor(self.xp.tanh(self.data), [self], "tanh", requires_grad=self.requires_grad, device=self.device)

    def sin(self):
        return Tensor(self.xp.sin(self.data), [self], "sin", requires_grad=self.requires_grad, device=self.device)

    def cos(self):
        return Tensor(self.xp.cos(self.data), [self], "cos", requires_grad=self.requires_grad, device=self.device)
    ...
    def maximum(self, t):
        ...
        return Tensor(self.xp.maximum(self.data, t.data), args, "maximum", requires_grad=requires_grad, device=self.device)
    ...
    def minimum(self, t):
        ...
        return Tensor(self.xp.minimum(self.data, t.data), args, "minimum", requires_grad=requires_grad, device=self.device)

    def max(self, axis = None, keepdims = False): #equivalent to torch.amax
        return Tensor(self.data.max(axis = axis, keepdims = keepdims), [self, axis, keepdims], "max", requires_grad=self.requires_grad, device=self.device)

    def min(self, axis = None, keepdims = False): #equivalent to torch.amin
        return Tensor(self.data.min(axis = axis, keepdims = keepdims), [self, axis, keepdims], "min", requires_grad=self.requires_grad, device=self.device)

    def concatenate(self, *tensors, axis = 0):
        tensors = [self.tensor(t) for t in tensors]
        return Tensor(self.xp.concatenate([self.data] + [t.data for t in tensors], axis = axis), [self] + tensors + [axis], "concatenate", requires_grad=self.requires_grad or any([t.requires_grad for t in tensors]), device=self.device)

    def reshape(self, *shape):
        shape = shape[0] if len(shape) == 1 else shape
        return Tensor(self.data.reshape(shape), [self], "reshape", requires_grad=self.requires_grad, device=self.device)
    ...
    def abs(self):
        return Tensor(self.xp.abs(self.data), [self], "abs", requires_grad=self.requires_grad, device=self.device)

    def transpose(self, *axes):
        axes = axes[0] if len(axes) == 1 else axes
        if len(axes) == 0:
            axes = range(self.data.ndim)[::-1]
        return Tensor(self.data.transpose(axes), [self, axes], "transpose", requires_grad=self.requires_grad, device=self.device)
    
    def swapaxes(self, axis1, axis2):
        return Tensor(self.xp.swapaxes(self.data, axis1, axis2), [self, axis1, axis2], "swapaxes", requires_grad=self.requires_grad, device=self.device)

    def flip(self, axis):
        if axis is None:
            axis = range(self.data.ndim)
        return Tensor(self.xp.flip(self.data, axis), [self, axis], "flip", requires_grad=self.requires_grad, device=self.device)

    def __neg__(self):
        return Tensor(-self.data, [self], "neg", requires_grad=self.requires_grad, device=self.device)

    def __pos__(self):
        return Tensor(self.data, [self], "pos", requires_grad=self.requires_grad, device=self.device)
    ...
    def __iter__(self):
        return iter(Tensor(self.data[i], [self, i], "getitem", requires_grad=self.requires_grad, device=self.device) for i in range(self.data.shape[0]))

    def __getitem__(self, index): # problem when use grad array indexes: example y[0].grad; non-leaf tensor; in torch it retain_grad
        return Tensor(self.data[index], [self, index], "getitem", requires_grad=self.requires_grad, device=self.device)
    ...
    def backward(self, grad = None):#grad=self.xp.array(1) # TODO: ASSERT GRAD SHAPE == DATA SHAPE, assert grad.device == self.device
        ...
        if grad.size != self.data.size or grad.ndim != self.data.ndim or grad.shape != self.data.shape: # reverse broadcast; TODO : MAYBE MOVE IT TO ANOTHER PLACE
            if self.data.size == 1:
                ...
                grad = grad.sum(axis=tuple(self.xp.where(self.xp.array(self.data.shape) != self.xp.array(grad.shape))[0].tolist()), keepdims=True)
            else:
                data_shape = (1,) * (grad.ndim - self.data.ndim) + self.data.shape
                axis = tuple(self.xp.where(self.xp.array(data_shape) != self.xp.array(grad.shape))[0].tolist())
        ...
        if self.op == "add":
            ...
            self.args[0].backward(self.xp.ones_like(self.args[0].data) * grad / self.xp.prod(self.xp.array(self.args[0].data.shape)[_axis]))
            ...
            _axis = list(axis) if isinstance(axis, tuple) else axis
            self.args[0].backward(self.xp.ones_like(self.args[0].data) * grad * 2 * (self.args[0].data - self.args[0].data.mean(axis=axis, keepdims=True)) / self.xp.prod(self.xp.array(self.args[0].data.shape)[_axis]))

        elif self.op == "power":
            self.args[0].backward(grad * self.args[1].data * self.args[0].data ** (self.args[1].data - 1))
            self.args[1].backward(grad * self.args[0].data ** self.args[1].data * self.xp.log(self.args[0].data))
            ...
            grads = self.xp.split(grad, self.xp.cumsum(self.xp.array([arg_shape[axis] for arg_shape in args_shapes]))[:-1].tolist(), axis=axis)
