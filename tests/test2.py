...
from nnmodel.activations import ActivationFunction, LeakyReLU
from nnmodel.layers import (
    AveragePooling2D,
    BatchNormalization,
    Conv2D,
    Conv2DTranspose,
...
X = np.arange(0.0, (batch_size * inputs_num * input_size * input_size)).reshape(
    (batch_size, inputs_num, input_size, input_size)
)
W = np.arange(0.0, (inputs_num * kernels_num * kernel_size * kernel_size)).reshape(
    (kernels_num, inputs_num, kernel_size, kernel_size)
)  # np.ones((kernels_num, inputs_num, kernel_size, kernel_size)) #np.random.normal(0, 1, (kernels_num, inputs_num, kernel_size, kernel_size))
