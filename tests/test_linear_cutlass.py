...
from neunet.nn.experimental import CUDACUTLASSLinear
from neunet.nn.layers import Linear

# Импортируйте ваши классы здесь (предположим, они в текущем файле или импортированы)
# from your_module import Linear, CUDACUTLASSLinear 

def test_cutlass_linear():
    # Настройки
    batch_size = 128
    in_features = 256
    out_features = 512
    ...
    print(f"Testing Linear Layer: Batch={batch_size}, In={in_features}, Out={out_features}")

    # 1. Инициализация слоев
    # Создаем стандартный слой (эталон)
    ref_layer = Linear(in_features, out_features, bias=True, device=device)
    # Создаем CUTLASS слой
    cutlass_layer = CUDACUTLASSLinear(in_features, out_features, bias=True, device=device)
    ...
    # Сравнение градиентов по входу (dX)
    # print(f"X_ref.grad: {X_ref.grad=}")
    # print(f"X_cutlass.grad: {X_cutlass.grad=}")
    ...
    # Сравнение градиентов по весам (dW)
    ...
    # Сравнение градиентов по смещению (db)
    ...
    if all([forward_passed, grad_X_passed, grad_W_passed, grad_b_passed]):
        print("\n✨ ALL TESTS PASSED! CUTLASS implementation is correct.")
    else:
