...
for epoch in range(100):
    ...
    for i in tqdm_range:
        ...
        real_data_loss = loss(
            ...
            torch.ones((real_data_prediction.shape[0], 1)).to("cuda:0"),
        )
        ...
        fake_data_loss = loss(
            ...
            torch.zeros((fake_data_prediction.shape[0], 1)).to("cuda:0"),
        )
        ...
        fake_data_loss = loss(
            fake_data_prediction,
            torch.ones((fake_data_prediction.shape[0], 1)).to("cuda:0"),
        )
        fake_data_loss.backward()
        g_optimizer.step()
        ...
        d_loss = (
            -np.log(real_data_prediction.data.cpu().numpy()).mean()
            - np.log(1 - fake_data_prediction.data.cpu().numpy()).mean()
        )
