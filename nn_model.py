...
class Model:
    def __init__(self):
        ...
        self.loss_functions = {
            "MSE": lambda output, target: -(target - output),
            "binary crossentropy": lambda output, target: -target / output
            + (1 - target) / (1 - output),  # logloss
            "categorical crossentropy": lambda output, target: target / output,
            None: lambda output, target=None: output,
        }

        self.loss_functions_metrics = {
            "MSE": lambda output, target: np.power(target - output, 2),
            "binary crossentropy": lambda output, target: -target * np.log(output)
    ...
    def act_func_der(self, output, name):  # derivatives for gradient computation
        ...
        if name == "Sigmoid":
            ...
            return (
                0.5 * np.tanh(0.0356774 * np.power(output, 3) + 0.797885 * output)
                + (0.0535161 * np.power(output, 3) + 0.398942 * output)
                * np.power(sech(0.0356774 * np.power(output, 3) + 0.797885 * output), 2)
                + 0.5
            )
        elif name == None:
    ...
    def summary(self):
        ...
        print(
            "{:<13} {:<15} {:<20} {:<20}".format(
                "Layer", "Shape", "Activation func", "Weights Shape"
            )
        )
        print("================================================================")
        ...
        if self.topology[1]["type"] == "Dense":
            weights_shapes.append(
                f"({self.topology[0]['inputs num']}x{self.topology[1]['neurons num']})"
            )

            total_weights += (
                self.topology[0]["inputs num"] * self.topology[1]["neurons num"]
            )
        elif self.topology[1]["type"] == "Conv2d":
            weights_shapes.append(
                f"({self.topology[1]['kernels num']}x{self.topology[1]['kernels size']}x{self.topology[1]['kernels size']})"
            )

            total_weights += (
                self.topology[1]["kernels num"]
                * self.topology[1]["kernels size"]
                * self.topology[1]["kernels size"]
            )
        ...
        for k in range(1, len(self.topology) - 1):
            if self.topology[k + 1]["type"] == "Dense":
                ...
                if self.topology[k]["type"] == "Dense":
                    weights_shapes.append(
                        f"({self.topology[k]['neurons num']}x{self.topology[k+1]['neurons num']})"
                    )

                    total_weights += (
                        self.topology[k]["neurons num"]
                        * self.topology[k + 1]["neurons num"]
                    )

                elif (
                    self.topology[k]["pooling type"] == "MaxPooling"
                    or self.topology[k]["pooling type"] == "AvgPooling"
                ):
                    weights_shapes.append(
                        f"({self.topology[k]['kernels num'] * self.topology[k]['pool size'] * self.topology[k]['pool size']}x{self.topology[k+1]['neurons num']})"
                    )

                    total_weights += (
                        self.topology[k]["kernels num"]
                        * self.topology[k]["pool size"]
                        * self.topology[k]["pool size"]
                        * self.topology[k + 1]["neurons num"]
                    )
                    ...
                    weights_shapes.append(
                        f"({self.topology[k]['kernels num'] * self.topology[k]['conv size'] * self.topology[k]['conv size']}x{self.topology[k+1]['neurons num']})"
                    )
                ...
                weights_shapes.append(
                    f"({self.topology[k+1]['kernels num']}x{self.topology[k+1]['kernels size']}x{self.topology[k+1]['kernels size']})"
                )
        ...
        for k in range(len(self.topology)):
            if self.topology[k]["type"] == "Input":
                ...
                if self.topology[k + 1]["type"] == "Conv2d":
                    if self.topology[k]["input type"] == "2d":
                        print(
                            "{:<13} {:<15} {:<20} {:<20}".format(
                                self.topology[k]["type"],
                                f"({self.topology[k]['inputs num']}x{self.topology[k+1]['init input size']}x{self.topology[k+1]['init input size']})",
                                "-",
                                "-",
                            )
                        )
            ...
            if self.topology[k]["type"] == "Dense":
                print(
                    "{:<13} {:<15} {:<20} {:<20}".format(
                        self.topology[k]["type"],
                        f"({self.topology[k]['neurons num']})",
                        str(self.topology[k]["activation func"]),
                        weights_shapes[i],
                    )
                )
                ...
                if self.topology[k]["dropout rate"] != None:
                    print(
                        "{:<13} {:<15} {:<20} {:<20}".format(
                            f"Dropout {self.topology[k]['dropout rate']}", "-", "-", "-"
                        )
                    )
            ...
            if self.topology[k]["type"] == "Conv2d":
                print(
                    "{:<13} {:<15} {:<20} {:<20}".format(
                        self.topology[k]["type"],
                        f"({self.topology[k]['kernels num']}x{self.topology[k]['conv size']}x{self.topology[k]['conv size']})",
                        str(self.topology[k]["activation func"]),
                        weights_shapes[i],
                    )
                )
                ...
                if self.topology[k]["pooling type"] != None:
                    print(
                        "{:<13} {:<15} {:<20} {:<20}".format(
                            self.topology[k]["pooling type"],
                            f"({self.topology[k]['kernels num']}x{self.topology[k]['pool size']}x{self.topology[k]['pool size']})",
                            "-",
                            "-",
                        )
                    )
    ...
    def add_input_layer(self, inputs_number, input_type):
        ...
        if not (input_type == "1d" or input_type == "2d"):
            raise SystemExit('Please input "1d" or "2d" input types')

        self.topology.append(
            {
                f"type": "Input",
                "inputs num": inputs_number,
                "input type": input_type,
                "dropout rate": None,
                "activation func": None,
            }
        )
    ...
    def add_conv2d_layer(
        ...
        kernels_size,
        input_size,
        transposing_stride=1,
        upsampling_scale_factor=1,
        padding=0,
        ...
        if self.topology[-1]["type"] == "Dense":
            previous_kernels_number = (
                self.topology[-1]["neurons num"] // init_input_size ** 2
            )
        elif self.topology[-1]["type"] == "Conv2d":
            previous_kernels_number = self.topology[-1]["kernels num"]
        elif (
            self.topology[-1]["type"] == "Input"
            and self.topology[-1]["input type"] == "2d"
        ):
            previous_kernels_number = self.topology[-1]["inputs num"]
        elif (
            self.topology[-1]["type"] == "Input"
            and self.topology[-1]["input type"] == "1d"
        ):
            previous_kernels_number = (
                self.topology[-1]["inputs num"] // init_input_size ** 2
            )
        ...
        self.topology.append(
            {
                "type": "Conv2d",
                "kernels num": kernels_number,
                "previous kernels num": previous_kernels_number,
                "kernels size": kernels_size,
                "kernel per input": kernel_per_input,
                "conv size": conv_size,
                "input size": input_size,
                "init input size": init_input_size,
                "activation func": activation_func,
                "bias": bias,
                "padding": padding,
                "stride": transposing_stride,
                "upsampling scale factor": upsampling_scale_factor,
                "pooling type": None,
                "block size": None,
                "pool size": None,
                "pool inds": None,
    ...
    def add_pooling_layer(self, block_size, pooling_type):
        ...
        if self.topology[-1]["type"] == "Conv2d":
            ...
            self.topology[-1]["pool size"] = (
                self.topology[-1]["conv size"] // block_size
            )
        else:
    ...
    def prepare_targets(self, targets):
        ...
        if type(targets) is int:
            ...
            if last_layer_act_func == "Sigmoid":
                ...
                raise SystemExit(
                    f'Activation function with name "{self.topology[-1]["activation func"]}" is not found'
                )
    ...
    def weights_init(self):  # Done
        ...
        if self.topology[1]["type"] == "Dense":
            self.weights.append(
                np.random.normal(
                    0.0,
                    pow(self.topology[0]["inputs num"], -0.5),
                    (self.topology[0]["inputs num"], self.topology[1]["neurons num"]),
                )
            )
            ...
            if self.topology[i + 1]["type"] == "Dense":
                ...
                if self.topology[i]["type"] == "Dense":
                    self.weights.append(
                        np.random.normal(
                            0.0,
                            pow(self.topology[i]["neurons num"], -0.5),
                            (
                                self.topology[i]["neurons num"],
                    ...
                    self.weights.append(
                        np.random.normal(
                            0.0,
                            pow(
                                self.topology[i]["kernels size"]
                                * self.topology[i]["kernels size"],
                                -0.5,
                            ),
                            (
                                self.topology[i]["kernels num"]
                                * self.topology[i]["pool size"]
                                * self.topology[i]["pool size"],
                                self.topology[i + 1]["neurons num"],
                            ),
                        )
                    ...
                    for h in range(kernels_size):
                        for w in range(kernels_size):
                            ...
                            gradient[i][h][w] = np.sum(
                                conv_layer_error[i]
                                * input_layer[ker, h : h + conv_size, w : w + conv_size]
                            )
                    ...
                    for w in range(kernels_size):
                        for i in range(l, r):
                            gradient[ker][h][w] += np.sum(
                                conv_layer_error[ker]
                                * input_layer[i, h : h + conv_size, w : w + conv_size]
                            )
    ...
    @njit
    def BGD_optimizer(
        gradient, weights, k, alpha, beta, beta2, epsilon, v, m, v_hat, m_hat
    ):
    ...
    def Nadam_optimizer(
        ...
        m_hat = m / (1 - np.power(beta, k)) + (1 - beta) * gradient / (
            1 - np.power(beta, k)
        )
        v_hat = v / (1 - np.power(beta2, k))
    ...
    def make_padding(layer, padding_size):
        ...
        padded_layer = np.zeros(
            (
                layer.shape[0],
                layer.shape[1] + 2 * padding_size,
                layer.shape[2] + 2 * padding_size,
            )
        )
        ...
        for i in range(layer.shape[0]):
            padded_layer[
                i,
                padding_size : layer.shape[1] + padding_size,
                padding_size : layer.shape[1] + padding_size,
            ] = layer[i]
    ...
    def make_upsampling(layer, scale_factor):  # njit doesnt work
        ...
        upsampled_layer = np.zeros(
            (
                layer.shape[0],
                layer.shape[1] * scale_factor,
                layer.shape[2] * scale_factor,
            )
        )
        ...
        for k in range(layer.shape[0]):
            for i in range(layer.shape[1]):
                for j in range(layer.shape[2]):
                    upsampled_layer[
                        k,
                        i + ic : (i + 1) * scale_factor,
                        j + jc : (j + 1) * scale_factor,
                    ] = layer[k, i, j]
        ...
        for k in range(len(self.topology) - 1, 0, -1):
            ...
            for l in range(len(outputs)):
                ...
                if self.topology[k]["type"] == "Dense":
                    ...
                    gradient += np.dot(
                        input_layer, losses[l][k]
                    )  # * self.act_func_der(outputs[l][k], self.topology[k]['activation func']) внутри скобки

                    # optimizer(gradient, [k])

                elif self.topology[k]["type"] == "Conv2d":

                    input_layer = self.reshape_to_2d(
                        outputs[l][k - 1],
                        num=self.topology[k]["previous kernels num"],
                        size=self.topology[k]["init input size"],
                    )

                    # upsampling
                    if self.topology[k]["upsampling scale factor"] != 1:
                        input_layer = self.make_upsampling(
                            input_layer, self.topology[k]["upsampling scale factor"]
                        )
                    # transposing
                    if self.topology[k]["stride"] != 1:
                        input_layer = self.make_transposing(
                            input_layer, self.topology[k]["stride"]
                        )
                    # padding
                    if self.topology[k]["padding"] != 0:
                        input_layer = self.make_padding(
                            input_layer, self.topology[k]["padding"]
                        )

                    gradient += self.compute_conv_gradients(
                        input_layer,
                        losses[l][k],
                        self.weights[k - 1],
                        self.topology[k]["previous kernels num"],
                        self.topology[k]["kernels num"],
                        self.topology[k]["conv size"],
    ...
    @staticmethod
    @njit
    def conv_backward_prop(
        conv_layer_error, weights, kernels_num, input_size, conv_size, kernels_size
    ):
        ...
        if conv_size >= kernels_size:
            error_pattern = np.zeros(
                (kernels_num, input_size + conv_size - 1, input_size + conv_size - 1)
            )
        else:
            error_pattern = np.zeros(
                (
                    kernels_num,
                    input_size + kernels_size - 1,
                    input_size + kernels_size - 1,
                )
            )
        ...
        for i in range(kernels_num):
            error_pattern[
                i,
                kernels_size - 1 : conv_size + kernels_size - 1,
                kernels_size - 1 : conv_size + kernels_size - 1,
            ] = conv_layer_error[
                i
            ]  # Матрица ошибок нужного размера для прогона по ней весов
        ...
        for s in range(kernels_num):
            for h in range(input_size):
                for w in range(input_size):
                    ...
                    conv_layer_backward_prop_error[s, h, w] = np.sum(
                        error_pattern[s, h : h + kernels_size, w : w + kernels_size]
                        * weights_rot_180[s]
                    )
        ...
        if prev_kernels_num <= kernels_num:
            for k in range(
                ...
                for s in range(l, r):
                    ...
                    pooling_layer_error[k] += (
                        loss_wrong_num[s] * der_input_layer[k]
                    )  # * self.act_func_der(pooling_layer[k], self.topology[index - 1]['activation func']) #index
            ...
            for k in range(kernels_num):
                for s in range(l, r):
                    ...
                    pooling_layer_error[s] = (
                        loss_wrong_num[k] * der_input_layer[k]
                    )  # * self.act_func_der(pooling_layer[s], self.topology[index - 1]['activation func']) #index
    ...
    @staticmethod
    @njit
    def pooling_error_expansion(
        pooling_layer_ind, pooling_layer_error, kernels_num, conv_size
    ):
    ...
    @njit
    def pooling(
        conv_layer, kernels_num, conv_size, pool_size, block_size, pooling_type
    ):
            ...
            for h in range(pool_size):
                for w in range(pool_size):
                    ...
                    pool_part = conv_layer[
                        ...
                        w * block_size : w * block_size + block_size,
                    ]  # .astype(np.float64)
                    ...
                    for h in range(conv_size):
                        for w in range(conv_size):
                            ...
                            conv_layer[i, h, w] = (
                                np.sum(
                                    input_layer[
                                        k, h : h + kernels_size, w : w + kernels_size
                                    ]
                                    * weights[i]
                                )
                                + bias
                    ...
                    for h in range(conv_size):
                        for w in range(conv_size):
                            ...
                            conv_layer[k, h, w] += (
                                np.sum(
                                    input_layer[
                                        i, h : h + kernels_size, w : w + kernels_size
                                    ]
                                    * weights[k]
                                )
                                + bias
    ...
    def forward_prop(self, input_values):
        ...
        for k in range(0, len(self.topology) - 1):
            ...
            if self.topology[k + 1]["type"] == "Dense":
                ...
                if (
                    self.topology[k]["type"] == "Dense"
                    or self.topology[k]["type"] == "Input"
                ) and self.topology[k]["dropout rate"] != None:
                    outputs[k] *= np.random.binomial(
                        n=1,
                        p=1 - self.topology[k]["dropout rate"],
                        size=outputs[-1].shape,
                    )

                outputs.append(
                    self.activation_func(
                        np.dot(np.array(outputs[k].flatten(), ndmin=2), self.weights[k])
                        + self.topology[k + 1]["bias"],
                ...
                if (
                    ...
                    input_layer = self.reshape_to_2d(
                        outputs[k],
                        num=self.topology[k + 1]["previous kernels num"],
                        size=self.topology[k + 1]["init input size"],
                    )

                elif (
                    self.topology[k]["type"] == "Conv2d"
                    or self.topology[k]["pooling type"] != None
                ):
                ...
                if self.topology[k + 1]["upsampling scale factor"] != 1:
                    input_layer = self.make_upsampling(
                        input_layer, self.topology[k + 1]["upsampling scale factor"]
                    )
                # transposing
                if self.topology[k + 1]["stride"] != 1:
                    input_layer = self.make_transposing(
                        input_layer, self.topology[k + 1]["stride"]
                    )
                # padding
                if self.topology[k + 1]["padding"] != 0:
                    input_layer = self.make_padding(
                        input_layer, self.topology[k + 1]["padding"]
                    )
                ...
                if self.topology[k + 1]["pooling type"] != None:
                    outputs[-1], self.topology[k + 1]["pool inds"] = self.pooling(
                        outputs[-1],
                        self.topology[k + 1]["kernels num"],
                        self.topology[k + 1]["conv size"],
    ...
    def backward_prop(self, layers_outputs, loss_func):
        ...
        losses[-1] = loss_func * self.act_func_der(
            layers_outputs[-1], self.topology[-1]["activation func"]
        )

        if self.topology[-1]["type"] == "Conv2d":
            if self.topology[-1]["pooling type"] != None:
                losses[-1] = self.pooling_error_expansion(
                    self.topology[-1]["pool inds"],
                    self.reshape_to_1d(
                        losses[-1],
                        num=self.topology[-1]["kernels num"],
                        size=self.topology[-1]["pool size"],
                    ),
                    self.topology[-1]["kernels num"],
        ...
        for k in range(len(self.topology) - 2, -1, -1):  # 0
            ...
            if (
                ...
                self.topology[k + 1]["type"] == "Dense"
            ):  # input
                losses[k] = (
                    np.dot(losses[k + 1], self.weights[k].T)
                ) * self.act_func_der(
                    layers_outputs[k].flatten(), self.topology[k]["activation func"]
                )

            elif (
                (self.topology[k]["type"] == "Conv2d")
                and (self.topology[k]["pooling type"] == None)
                and (self.topology[k + 1]["type"] == "Dense")
            ):
                losses[k] = self.reshape_to_2d(
                    np.dot(losses[k + 1], self.weights[k].T)
                    * self.act_func_der(
                        layers_outputs[k], self.topology[k]["activation func"]
                    ).flatten(),
                    num=self.topology[k]["kernels num"],
                    size=self.topology[k]["conv size"],
                )
                ...
                pooling_losses = self.reshape_to_1d(
                    np.dot(losses[k + 1], self.weights[k].T)
                    * self.act_func_der(
                        layers_outputs[k], self.topology[k]["activation func"]
                    ).flatten(),
                    num=self.topology[k]["kernels num"],
                    size=self.topology[k]["pool size"],
                )

                losses[k] = self.pooling_error_expansion(
                    self.topology[k]["pool inds"],
                    pooling_losses,
                    self.topology[k]["kernels num"],
                    self.topology[k]["conv size"],
                )
                ...
                loss_wrg_num = self.conv_backward_prop(
                    losses[k + 1],
                    self.weights[k],
                    self.topology[k + 1]["kernels num"],
                ...
                der_reshaped_outputs = self.act_func_der(
                    self.reshape_to_2d(
                        layers_outputs[k],
                        num=self.topology[k + 1]["previous kernels num"],
                        size=self.topology[k + 1]["init input size"],
                    ),
                    self.topology[k]["activation func"],
                )
                # upsampling
                if self.topology[k + 1]["upsampling scale factor"] != 1:
                    der_reshaped_outputs = self.make_upsampling(
                        der_reshaped_outputs,
                        self.topology[k + 1]["upsampling scale factor"],
                    )
                # transposing
                if self.topology[k + 1]["stride"] != 1:
                    der_reshaped_outputs = self.make_transposing(
                        der_reshaped_outputs, self.topology[k + 1]["stride"]
                    )
                # padding
                if self.topology[k + 1]["padding"] != 0:
                    der_reshaped_outputs = self.make_padding(
                        der_reshaped_outputs, self.topology[k + 1]["padding"]
                    )

                losses[k] = self.loss_convertation(
                    loss_wrg_num,
                    der_reshaped_outputs,
                    self.topology[k + 1]["previous kernels num"],
                    self.topology[k + 1]["kernels num"],
                ...
                # unpading
                if self.topology[k + 1]["padding"] != 0:
                    losses[k] = self.make_unpadding(
                        losses[k], self.topology[k + 1]["padding"]
                    )
                # untransposing
                if self.topology[k + 1]["stride"] != 1:
                    losses[k] = self.make_untransposing(
                        losses[k], self.topology[k + 1]["stride"]
                    )
                # downsampling
                if self.topology[k + 1]["upsampling scale factor"] != 1:
                    losses[k] = self.make_downsampling(
                        losses[k], self.topology[k + 1]["upsampling scale factor"]
                    )
        ...
        for i in range(epochs):
            ...
            for j in tqdm_range:
                for k in range(len(batches[j])):
                    ...
                    layers_losses = self.backward_prop(
                        layers_outputs,
                        loss_function(
                            layers_outputs[-1],
                            targets_list.reshape(layers_outputs[-1].shape),
                        ),
                    )
                ...
                tqdm_range.set_description(
                    f"training | optimizer: {optimizer_name} | loss: {loss:.4f} | epoch {i + 1}/{epochs}"
                )

                self.weights_updating(
                    batch_layers_outputs, batch_layers_losses, optimizer
                )
