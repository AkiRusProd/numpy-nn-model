...
class VAE:
    ...
    def encoder_forward_prop(self, inputs):
        ...
        for k in range(len(inputs)):
            ...
            reparametrized_outputs, mean, log_variance, epsilon = self.reparametrize(
                outputs[-1][0]
            )
        ...
        return (
            batch_layers_outputs,
            np.asarray(batch_layers_last_outputs),
            np.asarray(means),
            np.asarray(log_variances),
            np.asarray(epsilons),
        )
    ...
    def reparametrize(self, outputs):
        ...
        epsilon = np.random.normal(
            0, 1, (log_variance.shape)
        )  # (1, self.latent_dim_size)
    ...
    def encoder_backward_prop(
        self,
        batch_layers_outputs,
        batch_decoder_losses,
        batch_means,
        batch_log_variances,
        batch_epsilons,
    ):
        ...
        for k in range(len(batch_layers_outputs)):
            ...
            rec_log_var_loss = (
                batch_decoder_losses[k][0]
                * 0.5
                * np.exp(batch_log_variances[k] * 0.5)
                * batch_epsilons[k]
            )  # np.random.normal(0, 1, ((batch_log_variances[k].shape))) #(1, self.latent_dim_size)
        ...
        self.encoder.weights_updating(
            batch_layers_outputs, batch_layers_losses, self.encoder_optimizer
        )
    ...
    def decoder_backward_prop(self, batch_layers_outputs, batch_targets):
        ...
        for k in range(len(batch_layers_outputs)):
            ...
            losses = self.decoder.backward_prop(
                batch_layers_outputs[k],
                self.decoder_loss_function(
                    batch_layers_outputs[k][-1], batch_targets[k]
                ),
            )
            batch_layers_losses.append(losses)

        self.decoder.weights_updating(
            batch_layers_outputs, batch_layers_losses, self.decoder_optimizer
        )
    ...
    def train(
        ...
        **optimizer_params,
        ...
        self.decoder_loss_function_metric = self.decoder.loss_functions_metrics[
            loss_function_name
        ]
        ...
        for i in range(epochs):
            ...
            for j in tqdm_range:
                ...
                (
                    ...
                    log_variances,
                    epsilons,
                ) = self.encoder_forward_prop(inputs_batches[j])
                decoder_outputs, decoder_last_outputs = self.decoder_forward_prop(
                    reparametrized_outputs
                )

                decoder_losses = self.decoder_backward_prop(
                    decoder_outputs, targets_batches[j]
                )
                self.encoder_backward_prop(
                    encoder_outputs, decoder_losses, means, log_variances, epsilons
                )

                decoder_loss = self.decoder_loss_function_metric(
                    np.asfarray(decoder_last_outputs), targets_batches[j]
                ).mean()
                kl_loss = (
                    -0.5
                    * np.sum(
                        1 + log_variances - np.power(means, 2) - np.exp(log_variances)
                    )
                ...
                tqdm_range.set_description(
                    f"training | optimizer: {optimizer_name} | loss: {loss:.4f} | decoder loss: {decoder_loss:.4f} |kl loss: {kl_loss:.4f} | epoch {i + 1}/{epochs}"
                )  #
